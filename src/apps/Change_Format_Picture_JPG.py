...
def change_format_to_jpg_and_delete(folder_path):
    ...
    for main_path, sub_folders, files in os.walk(folder_path):
        for name_file in files:
            # Verifica si el archivo es una imagen y no es JPG ni GIF
            if name_file.lower().endswith(('.png', '.bmp', '.webp')) and not name_file.lower().endswith(('.jpg', '.jpeg', '.gif')):
                ...
                try:
                    ...
                    # Crea nueva ruta con extensión JPG
                    new_path_file = os.path.join(main_path, f"{name_without_extension}.jpg")
...
if __name__ == "__main__":
    ...
    change_format_to_jpg_and_delete(original_folder_path)
